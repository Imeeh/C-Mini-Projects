...
void window(){
    ...
    gotoxy(28,2);
    SetColor(35);
    printf("STUDENT MIS SOLUTION");
    gotoxy(20,3);
    printf("Parallel Algorithm Systems Poland");
    gotoxy(31,4);
    printf("Gdansk.: 2019");
    gotoxy(25,24);
    SetColor(17);
...
void use_pass_field(){
    ...
    char pass[10];
    SetColor(10);
    gotoxy(15,12);printf("This database is password protected.");
    gotoxy(15,13);printf(" Enter a valid username and password");
    SetColor(17);
...
int confid_record(char id[]){
   // left for you
   //Checks if the inputed id for
   //new record or if it's already in the database.
}
...
void add_student(){
    ...
    SetColor(45);
    if(fp == NULL){
        MessageBox(0,"Error while opening the file\Make sure your file is not read only","Warning",0);

    }else{
        fflush(stdin);
        gotoxy(print,10);printf("SID: ");gets(stu.SID);
        //here you can confirms the SID
        ...
        gotoxy(print,18);printf("Course: ");scanf("%d",&stu.Course);
...
void search_student(){
    ...
    char s_id[15];
    int isFound = 0;
    gotoxy(37,10);printf("Enter SID to Search: ");fflush(stdin);
    gets(s_id);
    FILE *fp;
...
void mod_student(){
    ...
    gotoxy(37,10);printf("Enter SID to Modify: ");fflush(stdin);
    gets(s_id);
    FILE *fp;
    fp = fopen("record.txt","rb+");
    while(fread(&stu, sizeof(stu),1,fp) == 1){
    ...
    if(!isFound){
        gotoxy(print, 12);printf("Record not Found");
    }
    fclose(fp);
    SetColor(28);
    return;
...
void delete_student(){
    ...
    gotoxy(37,12);printf("Record successfully deleted");
    SetColor(28);
    return;
}
...
void main_window(){
    ...
    while(1){
        ...
        gotoxy(x,22);printf("Input your choice 1 - 7: ");
        scanf("%d",&choice);
        switch(choice){
            case 1:
                add_student();
...
int main(){
    ...
    SetConsoleTitle("Parallel-Algorithm.solutions.com - Student Management Information System");
    window();
    //use_pass_field();
    main_window();
    return 0;
